...
class Optimizer:
    def __init__(
        self, algorithm_name, coordinates, labels, optimization_metric
    ) -> None:
    ...
    def __optimize_dbscan(self, best_score):
        ...
        for eps in epsilons:
            for min_point in min_points:
                for distance_metric in distance_metrics:
                    print(
                        f"eps: {eps} | min_points: {min_point} | distance metric: {distance_metric} -> ",
                        end="",
                    )
                    dbscan = DBSCAN(
                        self.__coordinates,
        ...
        for seed in range(1, 4):
            ...
            try:
                gaussian_mixture = GaussianMixture(
                    self.__coordinates, self.__labels, n_clusters=clusters_no, seed=seed
                )
                gaussian_mixture.fit_transform()

                scorer = Scorer(
                    self.__coordinates,
                    gaussian_mixture.predicted_labels,
                    labels=self.__labels,
                    display=False,
                )
                score = scorer.get_score(self.__metric)
